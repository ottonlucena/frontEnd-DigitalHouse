...
function listPaddockManagersByName() {
  return paddockManagers
    .map((managers) => `${managers.name}, ${managers.taxNumber}`)
    .sort();
}
...
function sortPaddockTypeByTotalArea() {
  //const data = paddockType.concat(paddocks);
  /* const result = paddocks.reduce((total, acc) => total + acc.area, 0);
  console.log(result); */

  /* return data.map((type) => ({
    paddockTypeId: type.paddockTypeId,
    area: type.area,
  })); */
  const paddocksFilter = paddocks.reduce((acc, current) => {
    let x = paddocks.find(
      (item) => item.paddockTypeId === current.paddockTypeId
    );
    console.log(x);
  });
}
